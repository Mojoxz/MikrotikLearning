...
export default function AdminNilaiPage() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Laporan Nilai</h1>
        <p className="text-gray-500 text-sm">Rekap nilai semua murid</p>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">Memuat...</td></tr>
              : hasil.length === 0 ? <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">Belum ada data nilai</td></tr>
              : hasil.map((h, i) => (
                <tr key={h.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-500">{i + 1}</td>
                  <td className="px-4 py-3">
                    <div className="font-medium">{h.user?.nama}</div>
                    <div className="text-xs text-gray-400">{h.user?.email}</div>
                  </td>
                  <td className="px-4 py-3">
                    <div>{h.kuis?.judul}</div>
                    <div className="text-xs text-gray-400">{h.kuis?.materi?.judul}</div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`font-bold text-lg ${nilaiColor(h.nilai)}`}>{h.nilai.toFixed(1)}</span>
                  </td>
