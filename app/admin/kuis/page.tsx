...
interface Kuis {
  id: number; judul: string; durasi: number; isAktif: boolean; createdAt: string
  materi?: { judul: string }; guru?: { nama: string }; _count?: { soal: number }
}
interface Materi { id: number; judul: string; kategori: string }
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Kuis</h1>
          <p className="text-gray-500 text-sm">Kelola kuis dan soal</p>
        </div>
        <div className="flex gap-2">
          <a href="/admin/soal" className="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-200 border">Kelola Soal</a>
          <button onClick={openCreate} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">+ Tambah Kuis</button>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">Memuat...</td></tr>
              : kuis.length === 0 ? <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">Tidak ada data</td></tr>
              : kuis.map((k, i) => (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select required={!edit} value={form.materiId} onChange={e => setForm(f => ({ ...f, materiId: e.target.value }))}
              ...
              {materi.map(m => <option key={m.id} value={m.id}>{m.judul} ({m.kategori})</option>)}
            </select>
          </div>
          <div>
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="isAktif" checked={form.isAktif} onChange={e => setForm(f => ({ ...f, isAktif: e.target.checked }))} className="w-4 h-4" />
            <label htmlFor="isAktif" className="text-sm text-gray-700">Kuis Aktif (dapat dikerjakan murid)</label>
          </div>
