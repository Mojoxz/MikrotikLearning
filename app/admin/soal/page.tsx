...
interface Kuis { id: number; judul: string; materi?: { judul: string } }
...
export default function AdminSoalPage() {
  ...
  const openCreate = () => {
    setEdit(null)
    setForm({ kuisId: filterKuis, pertanyaan: '', pilihanA: '', pilihanB: '', pilihanC: '', pilihanD: '', jawabanBenar: 'A', bobotNilai: '10' })
    setIsOpen(true)
  }

  const openEdit = (s: Soal) => {
    setEdit(s)
    setForm({ kuisId: s.kuisId.toString(), pertanyaan: s.pertanyaan, pilihanA: s.pilihanA, pilihanB: s.pilihanB, pilihanC: s.pilihanC, pilihanD: s.pilihanD, jawabanBenar: s.jawabanBenar, bobotNilai: s.bobotNilai.toString() })
    setIsOpen(true)
  }
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openCreate} disabled={!filterKuis} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50">+ Tambah Soal</button>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-4">
        ...
        <select value={filterKuis} onChange={e => setFilterKuis(e.target.value)}
          ...
          <option value="">-- Pilih Kuis untuk melihat soal --</option>
